...
# Potential 
V0 = float(input("Enter the height of square potential barrier,e.g., V0 = 0.5\n V0 = ")) #defines potential barrier
print("Enter the range of barrier region, e.g., x1 = -5, x2 = 5")
x1,x2 = float(input("x1 = ")), float(input("x2 = "))            #Barrier Region in Space
V = np.where((x>= x1) & (x<=x2), V0,0)      #determines the potential region in which particle is moving
# Initial wave packet
...
#Probability density
line, = ax[0].plot(x, np.abs(psi)**2,label = '|ψ(x, t)|²', color='black') #Plots initial Probability density and stoes a reference line object
...
ax[0].plot(x,V/V0*0.2, 'grey', label='V(x) [scaled]') #potential plot

#Real part
line_real, = ax[1].plot(x,np.real(psi), label = 'Re[ψ]', color='red') #Plots initial Real part
...
ax[1].plot(x,V/V0*0.2, 'grey', label='V(x) [scaled]') #potential plot

#Imaginary part
ax[2].set_ylabel('Im(ψ(x, t))') #label for the imaginary part axis
line_imag, = ax[2].plot(x,np.imag(psi), label = 'Im[ψ]',color='blue') #Plots initial Imaginary part
...
ax[2].plot(x,V/V0*0.2, 'grey', label='V(x) [scaled]') #potential plot
